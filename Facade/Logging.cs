...
namespace job_scheduler_service.Facade
{
    /**
     * Logging
     * 
    ...
    public class Log
    {
        ...
        private static void WriteLog(string message, string logType)
        {
            ...
            string pathToLog = EnvironmentVariables.GetInstance().Key("default-log-path") + "/" + currentDateInString + ".log";

            FileStream stream;
            // Call out 4 & 4.1
            if (!File.Exists(pathToLog)) 
            {
                if (!Directory.Exists(EnvironmentVariables.GetInstance().Key("default-log-path")))
                {
                    Directory.CreateDirectory(EnvironmentVariables.GetInstance().Key("default-log-path"));
                }

                stream = File.OpenWrite(pathToLog);
